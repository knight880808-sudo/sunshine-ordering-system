...
T: dict[str, dict[str, str]] = {
    ...
    "ai_missing_key":  {"en": "Gemini is not configured. Set GEMINI_API_KEY in environment variables.",
                        "zh": "未配置Gemini。请先设置环境变量 GEMINI_API_KEY。"},
...
def _call_gemini_api(question: str, context_text: str) -> tuple[bool, str]:
    ...
    system_text = (
        f"{t('ai_role_only')}\n"
        "Use concise bilingual-friendly wording when useful.\n"
        "If data is missing, clearly say unknown instead of inventing.\n"
        "Never claim an action has been executed."
    )
    prompt = (
        f"{system_text}\n\n"
        f"[SystemContext]\n{_safe_text(context_text, 1800)}\n\n"
        f"[UserQuestion]\n{_safe_text(question, 800)}"
    )
    payload = {
    ...
    try:
        ...
        if not cands:
            return False, t("ai_error")
        parts = (cands[0].get("content") or {}).get("parts") or []
        answer = "\n".join((p.get("text") or "").strip() for p in parts).strip()
        if not answer:
            return False, t("ai_error")
        return True, _safe_text(answer, max_len=4000)
    except urllib.error.HTTPError as e:
        msg = f"HTTP {e.code}"
        try:
            detail = e.read().decode("utf-8", errors="replace")
            msg = f"{msg}: {_safe_text(detail, 300)}"
        except Exception:
            pass
        log_exception("call_gemini_api_http", Exception(msg))
        return False, t("ai_error")
    except Exception as e:
        log_exception("call_gemini_api", e)
        return False, t("ai_error")
...
def page_ai_assistant() -> None:
    ...
    with q1:
        quick_shortage = st.button(t("ai_q_shortage"), use_container_width=True, key="ai_q_shortage_btn")
    with q2:
        quick_not_found = st.button(t("ai_q_not_found"), use_container_width=True, key="ai_q_nf_btn")
    with q3:
        if st.button(t("ai_clear_chat"), use_container_width=True, key="ai_clear_btn"):
            st.session_state["ai_chat"] = []
            st.rerun()

    quick_dispatch = st.button(t("ai_q_dispatch"), use_container_width=True, key="ai_q_dispatch_btn")
    with_ctx = st.checkbox(t("ai_with_ctx"), value=True, key="ai_with_ctx")
    user_q = st.text_area(t("ai_question"), key="ai_input", height=110)
    ...
    if st.button(t("ai_ask_btn"), type="primary", use_container_width=True, key="ai_ask_submit"):
        q = (user_q or "").strip()
        if not q:
        ...
        ctx = _build_ai_context() if with_ctx else ""
        ok, ans = _call_gemini_api(q, ctx)
        chat = st.session_state.setdefault("ai_chat", [])
        chat.append({"q": q, "a": ans if ok else t("ai_error"), "ok": ok, "ts": now_str()})
        st.rerun()

    chat = st.session_state.get("ai_chat", [])
    if not chat:
        return
    st.markdown(f"#### {t('ai_answer')}")
...
def _branch_order_browse() -> None:
    ...
    now_ts = unix_ts()
    last_raw = st.session_state.get("_search_last_raw", "")
    if query_raw != last_raw:
        st.session_state["_search_last_raw"] = query_raw
        st.session_state["_search_last_change_ts"] = now_ts
    stable_query = st.session_state.get("_search_stable_query", "")
    last_change_ts = float(st.session_state.get("_search_last_change_ts", 0.0))
    if (now_ts - last_change_ts) >= 0.4:
        stable_query = query_raw
        st.session_state["_search_stable_query"] = stable_query
    elif query_raw != stable_query:
        st.caption(t("search_typing_hint"))
    query = stable_query
...
def page_branch_order_done() -> None:
    st.markdown(f"### ✅ {t('order_sent_done')}")
